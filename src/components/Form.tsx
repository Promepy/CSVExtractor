...
function Form() {
  ...
  const handleSeparatorChange = (event: any) => {
  ...
  return (
    <>
      ...
      <div className="m-8 text-white ">
        {data && <h2 className="mb-4 text-3xl font-bold">Extracted Results:</h2>}
        {data && (
          <DataTable tableStyle={{ minWidth: '60rem' }} value={data.preview_rows}>
            {data.header.map((header: string) => (
              <Column
                key={header}
                field={header}
                header={header}
